...
type gctx = {
  ...
  thickness: int; (*Current line thicknes*)
  }
...
(*Produce new Gctx with different line thickness*)
...
let set_graphics_state (gc: gctx) : unit =
  let c = gc.color in
  let t = gc.thickness in  (*Added*)
  Graphics.set_color (Graphics.rgb c.r c.g c.b);
  Graphics.set_line_width t (*Added*)
...
(*Draw a point, cenetered at position, with given graphics context*)
...
(*Draw each point, centered at position x, with given graphics context*)
